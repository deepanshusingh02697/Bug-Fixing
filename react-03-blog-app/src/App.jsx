...
const INITIAL_POSTS = [
  {
    ...
    excerpt:
      "An introduction to useState, useEffect, and useCallback in modern React applications.",
  ...
  },
  {
    ...
    excerpt:
      "Mastering asynchronous JavaScript with async/await and proper error handling patterns.",
  ...
  },
  {
    ...
    excerpt:
      "Building scalable and maintainable REST APIs with Node.js and Express.",
...
export default function App() {
  ...
  function handleLike(postId,counter) {
    ...
    setPosts((prev) =>
      prev.map((p) => {
        if (p.id === postId) {
          return { ...p,liked:counter===1?true:false, likes: p.likes + counter};
        }
        return p;
      }),
    );
    setSelectedPost((prev) => ({
      ...prev,
      liked: counter===1?true:false,
      likes: prev.likes + counter
    }));
  }
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        {view === "create" && <CreatePost onSubmit={addPost} />}
      </div>
    </div>
  );
}
...
function PostDetail({ post, onBack, onLike, onAddComment }) {
  ...
  function handleLikeState(id,liked) {
    if(!liked ){
      onLike(id,+1)
    }else{
      onLike(id,-1)
    ...
    <div className="post-detail">
      ...
      <div className="likes">
        <button
          ...
          onClick={() => handleLikeState(post.id,post.liked)}
        >
          ❤️ Like
        </button>
